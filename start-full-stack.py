#!/usr/bin/env python3
"""
AI SchoolOS Full Stack Starter
Starts both backend services and frontend development server
"""
...
class FullStackStarter:
    ...
    def install_frontend_dependencies(self) -> bool:
        """Install frontend dependencies with timeout and fallback methods"""
        print("📦 Installing frontend dependencies...")
        
        # First, try standard npm install with timeout
        try:
            print("🔄 Attempting standard npm install...")
            ...
            print("⚠️  npm install timed out, trying alternative methods...")
        ...
        for method in alternative_methods:
            try:
                ...
                print(f"✅ Frontend dependencies installed using: {' '.join(method)}")
                return True
            except subprocess.TimeoutExpired:
                print(f"⚠️  {' '.join(method)} timed out")
            except subprocess.CalledProcessError as e:
                print(f"❌ {' '.join(method)} failed: {e}")
        
        print("❌ All npm install methods failed")
        print("💡 Try running: python fix-npm-install.py")
        return False
...
"""
    ...
    def run(self, backend_services: List[str] = None, install_deps: bool = True):
        """Main run method"""
        if backend_services is None:
            backend_services = self.default_backend_services
            
        print("🚀 Starting AI SchoolOS Full Stack...")
        print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(description="AI SchoolOS Full Stack Starter")
